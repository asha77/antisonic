...
def obtain_device_family(vendor, config):
    '''
    Extract device family: cisco_catalyst, cisco_isr, cisco_nexus, arista_switch, cisco_vrouter, huawei_ce, huawei_s, huawei_ar, edgecore_broadcom
    '''
    if vendor == 'cisco':
    ...
    if vendor == 'edgecore':
        match = re.search('accton_as', config)
        if match:
            return 'edgecore_broadcom'
...
def obtain_device_os(vendor_id, config):
    '''
    Extract software family from show version: cisco_ios_xe, cisco_ios, cisco_ios_xr, arista_eos, cisco_nx_os, huawei_vrp, aruba_aoscx, sonic_edgecore
    '''
    if vendor_id == 'cisco':
    ...
    if vendor_id == 'edgecore':
        match = re.search('SONiC.Edgecore-SONiC', config)
        if match:
            return 'sonic_edgecore'
